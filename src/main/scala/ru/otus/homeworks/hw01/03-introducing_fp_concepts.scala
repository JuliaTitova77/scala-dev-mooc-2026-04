...
import ru.otus.module1.variance.Animal
...
object opt {
  ...
  sealed trait Option[+T] {
    ...
    def printIfAny():Unit =
      if (!isEmpty) println(this.asInstanceOf[Some[T]].v)
    ...
    def zip[U](other: Option[U]): Option[(T,U)] =
      if (isEmpty || other.isEmpty) None
      else Some((this.asInstanceOf[Some[T]].v, other.asInstanceOf[Some[U]].v))
...
object list {
  ...
  sealed trait List[+T]{
     ...
     */
    def reverse: List[T] = {
      def loop(source: List[T], target: List[T]): List[T] = source match{
        case Nil =>
          target
        case ::(head, tail) =>
          loop(tail,head::target)
      }
      loop(this,Nil)
    }
  ...
  object List {
     ...
     */
    def incList(f: List[Int]): List[Int] =
      f.map(x => x + 1)
     ...
     */
    def shoutString(f: List[String]): List[String] =
      f.map(x => "!" + x)
  }
