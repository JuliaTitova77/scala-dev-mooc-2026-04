...
import ru.otus.homework.hw05.higher_kinded_types.{listBindable, optBindable, tupleBindable, tupleF}
...
object higher_kinded_types{
  ...
  def tupleF[F[_]:Monad, A, B](fa: F[A], fb: F[B]): F[(A, B)] =
    {for{
      a<-fa
      b<-fb
    }yield(a,b)
